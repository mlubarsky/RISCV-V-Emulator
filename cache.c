...
uint32_t cache_lookup_dm(struct cache_st *csp, uint64_t addr) {
    ...
    b_index = (addr >> 2) & csp->block_mask;    // block size > 1
    b_base = addr - (addr % 16);
    ...
    if (slot->valid && (slot->tag == tag)) {
        ...
        if (csp->block_size == 1) {
            ...
            for (int i = 0; i < csp->block_size; i++) {
                data = *((uint32_t *) b_base + i);  // Gets the data for each block 
                slot->block[i] = data;
            }
            data = slot->block[b_index];    // Update data with the new index of a slot block
        } 
    }
...
uint32_t cache_lookup_sa(struct cache_st *csp, uint64_t addr) {
    ...
    uint64_t b_index = (addr >> 2) & csp->block_mask; // block size > 1
    uint64_t b_base =  addr - (addr % 16);
    ...
    if (!hit) {
        if (csp->block_size == 1) {        
            ...
            for (int i = 0; i < csp->block_size; i++) {
                slot->block[i] = *((uint32_t *) b_base + i);
                slot->tag = tag;
                slot->valid = true;
            }
        }
